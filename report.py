...
def __sample(scan: FloatScanBatch, pred: FloatSegmBatch, segm: FloatSegmBatch):
    ...
    z = random.randint(0, scan.size("Z") - 1)
    image = get_white(scan, n=n, z=z).unsqueeze(2).numpy() / 255
    liver_image = rgb_sample(scan, pred, segm, mode=("liver_error", "liver", "background"), n=n, z=z, data_format="HWC") / 255
    tumor_image = rgb_sample(scan, pred, segm, mode=("tumor_error", "tumor", "background"), n=n, z=z, data_format="HWC") / 255
    class_labels = {
        0: "background",
        1: "liver",
        2: "tumor"
