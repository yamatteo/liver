...
def train(shrink, model, train_dataset, valid_dataset, args):
    epoch = 0
    k = 0
    ...
    round_loss = 0
    round_scores = {}
    streams = [shrink, *model.streams]

    keys = targets = [None, ] * len(streams)
    inputs = [[None, ] * len(streams), ] * (len(streams) + 1)
    ...
    validation_round(streams, valid_dataset, args)
    while epoch < args.epochs:
        for i, stream in enumerate(streams):
            inputs[i + 1][i] = stream(inputs[i][i])
        if inputs[-1][-1] is not None:
            loss = loss_func(
                inputs[-1][-1],
                functional.one_hot(targets[-1].to(device=last_device), args.finals)
                .permute(0, 4, 1, 2, 3).to(dtype=torch.float32)
            ) / args.grad_accumulation_steps
            loss.backward()
            round_loss += loss.item()
            round_scores.update({keys[-1]: loss.item()})
        if keys[-1] is not None and (
                (keys[-1] + 1) % args.grad_accumulation_steps == 0
                or keys[-1] + 1 == len(train_dataset)):
            optimizer.step()
            optimizer.zero_grad()
        ...
        if keys[-1] == len(train_dataset) - 1:
            ...
            if (epoch + 1) % 20 == 0:
                ...
                torch.save(dict(
                    arch=args.arch,
                    shrink=shrink.state_dict(),
                    lrlo_model=model.state_dict(),
                ), args.models_path / "last.pth")
                torch.save(dict(
                    arch=args.arch,
                    shrink=shrink.state_dict(),
                    lrlo_model=model.state_dict(),
                ), args.models_path / f"CHECKPOINT{epoch:03}.pth")
...
@torch.no_grad()
def validation_round(streams, valid_dataset, args):
    first_device, last_device = streams[0].device, streams[-1].device
    scores = []
    samples = []
    for bundle in valid_dataset:
        pred = []
        for slice in bundle.slices(args.slice_height):
            input = slice.scan.unsqueeze(0).to(device=first_device)
            for stream in streams:
                input = stream(input)
            input = torch.argmax(input, dim=1)
            pred.append(input)
        pred = torch.cat(pred, dim=-1)[..., :bundle.segm.size(-1)]
        segm = bundle.segm.unsqueeze(0).clamp(0, 1).to(device=last_device)
        intersection = torch.sum(pred * segm).item() + 1
        union = torch.sum((pred + segm).clamp(0, 1)).item() + 1
        scores.append({"name": bundle.name, "value": intersection/union})
        for _ in range(4):
            samples.append(report.sample(
                bundle.scan.unsqueeze(0).detach().cpu().numpy(),
