...
def _dim_slices(t: Tensor, *, dim: int, length: int, mode: str, with_start=False) -> Iterator[Tensor]:
    size = t.size(dim)
    if size <= length:
        yield t, 0 if with_start else t
        return
    if mode == "jump":
    ...
    for start in indices:
        yield (
            torch.narrow(t, dim, start, length), start
            if with_start
            else torch.narrow(t, dim, start, length)
        )


def slices3d(t: Tensor, shape: int3d, modes: str3d, zstart: bool = False) -> Iterator[Tensor]:
    dims = [1, 2, 3]
    for x in _dim_slices(t, dim=dims[0], length=shape[0], mode=modes[0]):
        for y in _dim_slices(x, dim=dims[1], length=shape[1], mode=modes[1]):
            if zstart:
                yield from _dim_slices(y, dim=dims[2], length=shape[2], mode=modes[2], with_start=True)
            else:
...
def slices(t: Tensor, *, shape: int3d, modes: str3d, pad: str, zstart: bool = False) -> Iterator[Tensor]:
    ...
    """
    for t, z in slices3d(t, shape, modes, zstart=zstart):
        if pad == "scan":
            yield (
