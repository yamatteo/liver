...
opts = SimpleNamespace(
    sources_path=Path('/gpfswork/rech/otc/uiu95bi/preds'),
)
...
def main():
    ...
    train_cases, valid_cases = train_test_split([p.absolute() for p in opts.sources_path.iterdir()], test_size=0.10)
    train_dataset = Dataset(train_cases)
    train_loader = DataLoader(train_dataset, batch_size=2, shuffle=True, num_workers=4, pin_memory=torch.cuda.is_available())
    valid_dataset = Dataset(valid_cases)
    val_loader = DataLoader(valid_dataset, batch_size=2, num_workers=4, pin_memory=torch.cuda.is_available())
    ...
    for epoch in range(5):
        ...
        for batch_data in train_loader:
            step += 1
            inputs, labels = batch_data["wscan"].to(device), batch_data["mvi"].to(device)
            optimizer.zero_grad()
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    val_images, val_labels = val_data["wscan"].to(device), val_data["mvi"].to(device)
